...
contract ShibaTitans is Context, IERC20, Ownable, LockToken 
{
    ...
    uint256 public minETHValueBeforeSwapping = 1 * 10 ** 17;
    ...
    function setMinETHValueBeforeSwapping(uint256 amount) public onlyOwner {
        minETHValueBeforeSwapping = amount;
    }
    ...
    function multiSendTokens(address[] calldata addresses, uint256[] calldata amounts) public onlyOwner{
        ...
        require(addresses.length <= 1000, "Array too long");
        for(uint256 i=0; i < addresses.length; i++){
            _transfer(msg.sender, addresses[i], amounts[i]);// SPDX-License-Identifier: Unlicensed
